...
Sau đó thực thi các tập lệnh sau theo đúng thứ tự:  
1. `CreateTable_vn_units.sql` – tạo cấu trúc cơ sở dữ liệu.  
2. `ImportData_vn_units.sql` – nhập dữ liệu mẫu.  

Chọn các tập lệnh tương ứng với hệ quản trị cơ sở dữ liệu bạn đang sử dụng:
...
> Người dùng Oracle có thể sử dụng cơ sở dữ liệu hoặc schema hiện có, vì việc tạo database phụ thuộc vào từng môi trường triển khai.
...
## Dữ liệu mở rộng (Add-ons)
Bên cạnh bộ dữ liệu đơn vị hành chính, repository này còn cung cấp các gói dữ liệu mở rộng tùy chọn nhằm bổ sung thêm tính năng và khả năng sử dụng.
### Bộ dữ liệu GIS

[![image.png](https://i.postimg.cc/dVmQJSFg/image.png)](https://postimg.cc/k2GPcsBy)
Phần mở rộng GIS cung cấp dữ liệu hình học ranh giới hành chính cho các đơn vị hành chính của Việt Nam và hiện hỗ trợ:
- PostgreSQL/PostGIS
- MySQL
