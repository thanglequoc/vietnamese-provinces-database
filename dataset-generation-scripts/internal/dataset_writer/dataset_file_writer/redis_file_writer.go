...
const hsetDatasetMetadataTemplate string = "HSET dataset_metadata datasetVersion \"%s\" latestDecree \"%s\" generatedAt \"%s\" \n"
...
func writeRedisReadme(outputFolderPath string) error {
	return writeDatasetReadme(outputFolderPath,
		...
		[]string{
			...
			"",
			"`dataset_metadata` fields: `datasetVersion`, `latestDecree`, `generatedAt` (UTC, RFC 3339).",
			"",
			...
			"redis-cli HGETALL dataset_metadata",
			"```",
		})
}
