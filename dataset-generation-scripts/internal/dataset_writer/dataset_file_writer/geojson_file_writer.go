		...
		Features: []file_writer_dto.GeoJSONFeature{
			{
				...
				Properties: file_writer_dto.GeoJSONFeatureProperties{
					UnitName:     province.VNProvince.Name,
					UnitCode:     province.VNDSProvinceCode,
					UnitCodeName: province.VNProvince.CodeName,
		...
		Features: []file_writer_dto.GeoJSONFeature{
			{
				...
				Properties: file_writer_dto.GeoJSONFeatureProperties{
					UnitName:     ward.VNWard.Name,
					UnitCode:     ward.VNDSWardCode,
					UnitCodeName: ward.VNWard.CodeName,
...
func writeGeoJSONReadme(outputFolderPath string, executionTime time.Time) error {
	readmePath := filepath.Join(outputFolderPath, "README.md")
	const fence = "```"
	lines := []string{
		"# GeoJSON GIS Export",
		"",
		fmt.Sprintf("> Created at:  %s", executionTime.Format(time.RFC1123Z)),
		"",
	...
	if err := os.WriteFile(readmePath, []byte(content), 0644); err != nil {
		return fmt.Errorf("write geojson readme %s: %w", readmePath, err)
	}
