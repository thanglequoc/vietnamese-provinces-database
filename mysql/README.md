...
- `mysql_ImportData_vn_units.sql` — INSERT statements for regions, units, provinces, and wards (342.79 KB)

## Overview

The Vietnamese Provinces Database for MySQL. The import script populates:
...
1. Create the tables by running `mysql_CreateTables_vn_units.sql`.
...
WHERE ST_Within(ST_GeomFromText('POINT(105.8542 21.0285)', 4326), g.geom);
```

## GIS / GeoJSON

The `gis/` subfolder contains chunked GIS import scripts (`MySQL` `*-part-NN.sql`) plus a `.manifest` file listing the chunks in order. Import every chunk in order after the base import to populate `gis_provinces` and `gis_wards` with `bbox` and `geom`.
