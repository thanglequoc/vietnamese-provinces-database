...
- `postgres_ImportData_vn_units.sql` — INSERT statements for regions, units, provinces, and wards (343.04 KB)

## Overview

The Vietnamese Provinces Database for PostgreSQL. The import script populates:
...
1. Create the tables by running `postgres_CreateTables_vn_units.sql`.
...
The `gis/` subfolder contains chunked GIS import scripts (`PostgreSQL` `*-part-NN.sql`) plus a `.manifest` file listing the chunks in order. Import every chunk in order after the base import to populate `gis_provinces` and `gis_wards` with `bbox` and `geom`.
