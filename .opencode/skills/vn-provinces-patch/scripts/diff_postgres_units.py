...
header comment, all `--`/`/* */` comments, any GIS content, and the `vn_provinces_metadata`
provenance table (its `generated_at` changes on every regeneration). Row ordering is not
...
# Tables deliberately excluded from the diff. `vn_provinces_metadata` carries provenance
# (version/decree/generated_at) and its timestamp changes on every regeneration, so it
# must never produce a patch.
IGNORED_TABLES = {"vn_provinces_metadata"}
...
def main():
    ...
    header = (
        ...
        "-- Excluded: vn_provinces_metadata (provenance metadata)\n"
        "-- ====================================================================="
    )
    patch_sql = header + "\n" + patch_sql + "\n"
