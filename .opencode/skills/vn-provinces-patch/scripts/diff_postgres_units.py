...
header comment, all `--`/`/* */` comments, any GIS content, and the `dataset_metadata`
...
# Tables deliberately excluded from the diff. `dataset_metadata` carries provenance
# (version/decree/generated_at) and its timestamp changes on every regeneration, so it
# must never produce a patch.
IGNORED_TABLES = {"dataset_metadata"}
...
def main():
    ...
    header = (
        ...
        "-- Excluded: dataset_metadata (provenance metadata)\n"
        "-- ====================================================================="
    )
    patch_sql = header + "\n" + patch_sql + "\n"
