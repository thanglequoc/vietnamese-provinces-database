...
| `dataset_metadata` | 1 | Dataset version descriptor (dataset_version, latest_decree, generated_at) |
...
**`dataset_metadata`**: `dataset_version` (e.g. `"v5.1.0"`), `latest_decree` (e.g. `"30/2026/QH16"`), `generated_at` (UTC timestamp). Single row, sourced from `dataset-generation-scripts/version.txt`.
...
**Credentials are not hardcoded here.** Before running any MongoDB command, load them from the git-ignored `.env.agent` file at the repo root (create it from `.env.agent.example` if missing):
...
| Variable | Purpose |
|----------|---------|
| `MONGO_URI` | Full connection string for `mongosh` / `mongoimport` |
| `MONGO_ROOT_USER` / `MONGO_ROOT_PASSWORD` | Root credentials (used by the `docker exec` fallback) |
| `MONGO_HOST` / `MONGO_PORT` / `MONGO_DB_NAME` | Host / port / default database |

```bash
# Check if MongoDB is reachable through the tunnel
echo 'db.version()' | mongosh "$MONGO_URI" --quiet 2>/dev/null || echo "unreachable"
...
| Detail | Value |
|--------|-------|
| Connection String | `$MONGO_URI` (from `.env.agent`) |
...
| Authentication | Root user — credentials from `.env.agent` (`MONGO_ROOT_USER` / `MONGO_ROOT_PASSWORD`) |
| SSH Host | `machine.thanglequoc.xyz` |
| SSH Port | `22` |
| SSH User | `thanglequoc` |
| SSH Key | `~/.ssh/id_ed25519` |
...
ssh -i ~/.ssh/id_ed25519 thanglequoc@machine.thanglequoc.xyz \
  "docker exec mongodb mongosh -u "$MONGO_ROOT_USER" -p "$MONGO_ROOT_PASSWORD" --quiet --eval \"use('vn_provinces'); db.getCollection('provinces-gis').countDocuments()\""
```
...
0. Load credentials from the git-ignored `.env.agent` file (repo root): `set -a; source "$(git rev-parse --show-toplevel)/.env.agent"; set +a`
1. Check tunnel reachability first: `echo 'db.version()' | mongosh "$MONGO_URI" --quiet 2>/dev/null || echo "unreachable"`
...
2. `BootstrapDatasetMetadata()` — loads `version.txt` and inserts the single `dataset_metadata` row
...
5. `ReadAndGenerateSQLDatasets()` — non-GIS exports (SQL/JSON/MongoDB/Redis/Elasticsearch), each carrying the `dataset_metadata` version info
